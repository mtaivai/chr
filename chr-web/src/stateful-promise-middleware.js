...
const statefulPromiseMiddleware = ({ dispatch }) => {
    return next => action => {
        ...
        if (isPromise(action.payload)) {
            if (_LOG_INFO) console.log("Stateful action with Promise payload: " + action.type, action);
            ...
            action.payload.then(
                ...
                error => {
                    ...
                    if (_LOG_ERROR) console.error("Action creator Promise resolved with error", error);
            ...
            if (_LOG_INFO) console.log("No a stateful action (payload is not a Promise): " + action.type, action);
            return next(action);
        }
